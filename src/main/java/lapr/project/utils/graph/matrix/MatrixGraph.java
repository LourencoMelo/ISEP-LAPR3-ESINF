...
public class MatrixGraph<V,E> extends CommonGraph<V,E> {
    ...
    public Collection<Edge<V, E>> edges() {
        ...
        Collection <Edge<V, E>> collection = new ArrayList<>();

        if (numEdges == 0) return collection;

        for (int linha = 0; linha < edgeMatrix.length; linha++){
            for (int coluna=0; coluna < edgeMatrix[linha].length; coluna++){
                if (edgeMatrix[linha][coluna] != null) collection.add(edgeMatrix[linha][coluna]);
            }
        }
    ...
    public Collection<Edge<V, E>> outgoingEdges(V vert) {
        ...
        Collection<Edge<V,E>> collection = new ArrayList<>();
        int vertKey = key(vert);
        if (vertKey == -1) return collection;

        for (int i=0; i < numVerts; i++){
            if (edgeMatrix[vertKey][i] != null)
                collection.add(edgeMatrix[vertKey][i]);
        }
        return collection;
    }
